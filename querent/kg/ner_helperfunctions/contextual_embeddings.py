...
class EntityEmbeddingExtractor:
    def __init__(self, model, tokenizer, number_entity_pairs, number_sentences):
        ...
        try:
            ...
            self.reducer = umap.UMAP(
                init="random",
                n_neighbors=min(15, number_entity_pairs),
                min_dist=0.1,
                n_components=10,
                metric="cosine",
            )
            self.sentence_reducer = umap.UMAP(
                init="random",
                n_neighbors=min(15, number_sentences),
                min_dist=0.1,
                n_components=10,
                metric="cosine",
            )
            ...
            self.logger.error(
                f"Error Initializing Entity Embedding Extractor Class: {e}"
            )
    ...
    def extract_entity_embedding(self, entity, context):
        try:
            ...
            combined_embedding = torch.cat(
                (entity_embedding, sentence_embedding), dim=0
            )
    ...
    def append_if_not_present(
        self, item, item_embedding, all_items, all_embeddings, sentence=None
    ):
    ...
    def _update_pairs_with_embeddings(self, doc_entity_pairs):
        ...
        for inner_list in doc_entity_pairs:
            ...
            for pair in inner_list:
                ...
                pair_dict["entity1_embedding"] = self.reducer.transform(
                    [entity1_embedding.tolist()]
                )[0]
                pair_dict["entity2_embedding"] = self.reducer.transform(
                    [entity2_embedding.tolist()]
                )[0]
        ...
        try:
            for inner_list in doc_entity_pairs:
                for pair in inner_list:
                    ...
                    context = self._get_relevant_context(entity1, entity2, full_context)
                    entity1_embedding, _ = self.extract_entity_embedding(
                        entity1, context
                    )
                    entity2_embedding, _ = self.extract_entity_embedding(
                        entity2, context
                    )
                    self.append_if_not_present(
                        entity1,
                        entity1_embedding,
                        all_entities,
                        all_embeddings,
