...
class BERTLLM(BaseEngine):
    def __init__(self, input_queue: QuerentQueue, config: BERTLLMConfig):
        ...
        if self.fixed_entities and not self.sample_entities:
            raise ValueError(
                "If specific entities are provided, their types should also be provided."
            )
        if self.fixed_entities and self.sample_entities:
            self.entity_context_extractor = FixedEntityExtractor(
                fixed_entities=self.fixed_entities, entity_types=self.sample_entities
            )
        ...
        if self.fixed_relationships and not self.sample_relationships:
            raise ValueError(
                "If specific predicates are provided, their types should also be provided."
            )
        ...
        try:
            ...
            if content:
                if self.fixed_entities:
                    content = self.entity_context_extractor.find_entity_sentences(
                        content
                    )
                    print("--------------------------------", content)
            ...
            if self.sample_entities:
                doc_entity_pairs = self.entity_context_extractor.process_entity_types(
                    doc_entities=doc_entity_pairs
                )
            print("doc entities---------------------", doc_entity_pairs)
            if doc_entity_pairs:
                pairs_withattn = (
                    self.attn_scores_instance.extract_and_append_attention_weights(
                        doc_entity_pairs
                    )
                )
                print("-----------", pairs_withattn)
                if self.count_entity_pairs(pairs_withattn) > 1:
                    self.entity_embedding_extractor = EntityEmbeddingExtractor(
                        self.ner_model,
                        self.ner_tokenizer,
                        self.count_entity_pairs(pairs_withattn),
                ...
                pairs_withemb = self.entity_embedding_extractor.extract_and_append_entity_embeddings(
                    pairs_withattn
                )
                print("-----------", pairs_withemb)
                pairs_with_predicates = process_data(pairs_withemb, file)
                if self.enable_filtering == True and not self.entity_context_extractor:
                    cluster_output = self.triple_filter.cluster_triples(
                        pairs_with_predicates
                    )
                    ...
                    final_clustered_triples = (
                        self.triple_filter.filter_by_cluster_persistence(
                            pairs_with_predicates, cluster_persistence, cluster_labels
                        )
                    )
                    if final_clustered_triples is not None:
                        filtered_triples, _ = self.triple_filter.filter_triples(
                            final_clustered_triples
                        )
                    else:
                        filtered_triples, _ = self.triple_filter.filter_triples(
                        ...
                        self.logger.log(
                            f"Filtering in {self.__class__.__name__} producing 0 entity pairs. Filtering Disabled. "
                        )
                        self.logger.log(
                            f"Filtering in {self.__class__.__name__} producing 0 entity pairs. Filtering Disabled. "
                        )
                ...
                embedding_triples = semantic_extractor.generate_embeddings(
                    relationships
                )
                print(
                    "-------------------------------- embedding triples: {}".format(
                        embedding_triples
                    )
                )
                if self.sample_relationships:
                    embedding_triples = (
                        self.predicate_context_extractor.process_predicate_types(
                            embedding_triples
                        )
                    )
                for triple in embedding_triples:
                    ...
                    print(
                        "-------------------------------- Graph : {}".format(graph_json)
                    )
                    if graph_json:
                        current_state = EventState(
                            EventType.Graph, 1.0, graph_json, file
                        )
                        await self.set_state(new_state=current_state)
                    vector_json = TripleToJsonConverter.convert_vectorjson(triple)
                    if vector_json:
                        current_state = EventState(
                            EventType.Vector, 1.0, vector_json, file
                        )
                        await self.set_state(new_state=current_state)
