    ...
    async def process_tokens(self, data: IngestedTokens):
        ...
        try:
            ...
            if doc_entity_pairs:
                ...
                print("Contextual triples------------------------", filtered_triples)
                ...
                print("RDF Contextual Triples----------------------------",kgm.retrieve_triples())
                await self.set_state(new_state=current_state)
                mock_config = RelationshipExtractorConfig()
                semantic_extractor = RelationExtractor(mock_config)
                relationships, semantic_triples = semantic_extractor.process_tokens(EventState(EventType.ContextualTriples, 1.0, filtered_triples, filename))
                ...
                await self.set_state(new_state=current_state)
                embedding_triples = semantic_extractor.generate_embeddings(EventState(EventType.ContextualTriples, 1.0, relationships, filename))
