...
#     def insert_sentiment_graph_event(self, data):
#     # Base query components
#         base_query = """
#         MERGE (e:Entity {{name: $entity, type: $entity_type{symbol}}})
#         MERGE (s:Sentiment {{sentiment: $sentiment, score: $sentiment_score}})
#         MERGE (e)-[:HAS_SENTIMENT {{topic_type: $topic_type, title: $title{url}{publishedAt}}}]->(s)
#         """
        
#         # Optional components
#         symbol_component = ", symbol: $symbol" if "symbol" in data else ""
#         url_component = ", url: $url" if "url" in data else ""
#         publishedAt_component = ", publishedAt: $publishedAt" if "publishedAt" in data else ""
        
#         # Final query with optional components added
#         final_query = base_query.format(symbol=symbol_component, url=url_component, publishedAt=publishedAt_component)
        
#         # Execute the query
#         try:
#             self.execute_query(final_query, parameters=data)
#             print("Query Complete")
#         except Exception as e:
#             print("Exception while inserting query---", e)
