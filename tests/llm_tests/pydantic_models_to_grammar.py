...
def generate_gbnf_grammar(model: Type[BaseModel], processed_models: set, created_rules: dict) -> (list, bool, bool):
    ...
    for field_name, field_info in model_fields.items():
        if not issubclass(model, BaseModel):
            ...
            is_optional = field_info.required is False and get_origin(field_type) is Optional
...
def generate_field_text(field_name: str, field_type: Type[Any], model: Type[BaseModel], depth=1,
    ...
    indent = '    ' * depth
    field_info = model.__fields__.get(field_name)
    field_description = field_info.__doc__ if field_info and field_info.__doc__ else ""
...
def add_run_method_to_dynamic_model(model: Type[BaseModel], func: Callable):
    ...
    def run_method_wrapper(self):
        func_args = {name: getattr(self, name) for name in model.__fields__}
        return func(**func_args)
