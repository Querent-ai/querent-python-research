...
@pytest.mark.parametrize(
    "input_data,ner_model_name,rel_model_name,llm_class,expected_entities",
    [
        (
            "Nishant is working from Delhi. Ansh is working from Punjab.",
            "dslim/bert-base-NER",
...
async def test_bertllm_ner_tokenization_and_entity_extraction(
    ...
    await input_queue.put(
        IngestedTokens(file="dummy_1_file.txt", data="Puneet is working from Houston.")
    )
    await input_queue.put(IngestedTokens(file="dummy_2_file.txt", data="dummy"))
    await input_queue.put(
        IngestedTokens(file="dummy_2_file.txt", data=None, error="error")
    )
    ...
    entities = llm_instance._extract_entities_from_chunks(
        llm_instance._tokenize_and_chunk(ingested_data.data)
    )
